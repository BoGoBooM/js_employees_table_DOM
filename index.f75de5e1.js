...
headers.forEach((header, index)=>{
    header.addEventListener("click", (e)=>{
        ...
        const sortedRows = rows.map((row)=>{
            const value = index === 4 ? row.children[index].textContent.trim().replaceAll(",", "").replace("$", "") : row.children[index].textContent.trim();
            return {
                element: row,
                value
            };
        });
        if (currentSortColumn === index) sortDirection = sortDirection === "asc" ? "desc" : "asc";
        else {
            currentSortColumn = index;
            sortDirection = "asc";
        }
        sortedRows.sort((a, b)=>{
            if (!isNaN(Number(a.value))) return sortDirection === "asc" ? Number(a.value) - Number(b.value) : Number(b.value) - Number(a.value);
            else return sortDirection === "asc" ? a.value.localeCompare(b.value) : b.value.localeCompare(a.value);
        });
        tbody.innerHTML = "";
...
form.addEventListener("submit", (e)=>{
    ...
    let message = "";
    if (nameEmployee.length < 4) message = "Name is too short";
    if (age < 18 || age > 90) message = "Invalid age";
    if (position.length < 2) message = "Invalid position";
    if (salary <= 0) message = "Invalid salary";
    if (message !== "") {
